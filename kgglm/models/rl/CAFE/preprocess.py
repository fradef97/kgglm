...
from kgglm.models.rl.CAFE.cafe_utils import (load_kg, load_labels, save_embed,
                                              save_kg, save_path_count,
                                              save_user_products)
from kgglm.models.rl.CAFE.parser import parse_args
from kgglm.models.rl.PGPR.pgpr_utils import TRANSE, load_embed
...
def load_kg_embedding(dataset: str):
    ...
    for entity in ENTITY_LIST[dataset]:
        embeds[entity] = state_dict[entity].cpu().data.numpy()[:-1]  # remove last dummy embed with 0 values.
